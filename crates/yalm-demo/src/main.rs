...
#[derive(Parser)]
#[command(name = "yalm-demo", about = "YALM — Geometric Comprehension Demo")]
struct Cli {
...
fn print_header() {
    ...
    println!("  \u{2551}{:^width$}\u{2551}", "YALM \u{2014} Geometric Comprehension", width = BOX_W);
...
fn main() {
    ...
    let mode_str = match build_mode {
        yalm_engine::BuildMode::Equilibrium => "Equilibrium",
        yalm_engine::BuildMode::ForceField => "ForceField",
    };
    ...
    for q in &questions {
        let (answer, _distance, _connector) = yalm_engine::resolver::resolve_question(
            q,
            engine.space(),
            &dictionary,
            engine.structural(),
