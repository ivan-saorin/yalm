...
pub fn evaluate_genome(
    ...
    let primary_report = yalm_eval::evaluate(&engine_dict5, test5, dict5, &engine_params, &strategy);
    ...
    let cross_report = if primary_report.fitness >= cross_validation_threshold {
        if let (Some(d12), Some(t12)) = (dict12, test12) {
            ...
            engine12.train(d12);
            Some(yalm_eval::evaluate(&engine12, t12, d12, &engine_params, &strategy))
        } else {
            None
        }
    } else {
    ...
    let cross18_report = if primary_report.fitness >= cross_validation_threshold {
        if let (Some(d18), Some(t18)) = (dict18, test18) {
            ...
            Some(yalm_eval::evaluate(&engine18, t18, d18, &engine_params, &strategy))
        } else {
            None
        }
    } else {
