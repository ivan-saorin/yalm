...
use yalm_core::Comprehend;
use yalm_engine::Engine;
use yalm_evolve::analysis::run_full_analysis;
use yalm_evolve::fitness::build_trained_space;
use yalm_evolve::genome::Genome;
use yalm_evolve::runner::{evolve, resume, EvolutionConfig};
use yalm_parser::{parse_dictionary, parse_test_questions};

#[derive(Parser)]
#[command(name = "yalm-evolve", about = "Evolutionary self-improvement for YALM")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn cmd_run_best(results_dir: &PathBuf, dict_path: &PathBuf, test_path: &PathBuf) {
    ...
    yalm_eval::print_space_statistics(engine.space(), &dictionary);

    println!("\n=== Test Results ===\n");
    let report = yalm_eval::evaluate(&engine, &test_suite, &dictionary, &engine_params, &strategy);
