...
**Decision**: YALM learns from a closed dictionary where every word in every definition is itself defined. No external corpus.
...
**Trade-off**: Loses nuance. "Democracy" defined at ELI5 level becomes "a way for people to choose leaders by voting" — correct but shallow. This is acceptable because YALM tests comprehension (does it understand?), not depth of knowledge.
...
- **Composability**: SELF participates in cross-space queries the same way MATH or GRAMMAR does. "Can YALM do math?" routes to both SELF and MATH spaces.
- **Identity as geometry**: "YALM" is a point in SELF space, near "system", "geometric", "comprehension". Its capabilities are connectors: "can answer", "can describe". Its limitations are distances: far from "image", "translate". The geometry IS the self-model.
