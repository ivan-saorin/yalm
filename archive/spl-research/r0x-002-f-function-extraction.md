...
r0x-001 proved that GPT-2 embedding distances correlate with DAFHNE equilibrium distances for dict5. This experiment builds the actual transformation function f that extracts a usable geometric space from a neural network.
...
**Solution:** Use dict5 as the Rosetta Stone. DAFHNE's geometry provides *meaning-grounded* distances. GPT-2 provides *statistically-rich* distances. Align the two via Procrustes analysis on the shared vocabulary.
...
# dafhne_points: 51 x N_dafhne (from DAFHNE equilibrium)
# gpt2_points: 51 x 768 (from GPT-2 wte, projected to same dimensionality)

# First: reduce GPT-2 to same dimensionality as DAFHNE via PCA
from sklearn.decomposition import PCA
pca = PCA(n_components=N_dafhne)
gpt2_reduced = pca.fit_transform(gpt2_points)

# Procrustes: find rotation + scaling that best aligns GPT-2 to DAFHNE
...
Once aligned, project GPT-2 words NOT in dict5 into DAFHNE-compatible space:

```python
# Take GPT-2 embeddings for dict12 words (1005 words)
# Apply same PCA transform + Procrustes rotation
# These points are now in DAFHNE-compatible space WITHOUT running equilibrium
...
### Step 3: Validate Against DAFHNE dict12

Run DAFHNE equilibrium on dict12. Compare distances:
- Between dict12 DAFHNE distances and projected GPT-2 distances
...
# Compare with DAFHNE connector assignments
...
├── r0x_002_alignment.png      # dict5 points: DAFHNE vs projected GPT-2
...
If DEAD: NN and DAFHNE capture fundamentally different structure. Proceed to r0x-003 (SPL native, no NN bridge).
